...
class LocationsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (! Gate::allows('location_delete')) {
            ...
            $table->editColumn('storefront', function ($row) {
                if($row->storefront) { return '<a href="'. asset(env('UPLOAD_PATH').'/' . $row->storefront) .'" target="_blank"><img src="'. asset(env('UPLOAD_PATH').'/thumb/' . $row->storefront) .'"/>'; };
            });
    ...
    public function show($id)
    {
        ...
        $clinics = \App\Clinic::get()->pluck('nickname', 'id')->prepend(trans('global.app_please_select'), '');
        $contact_people = \App\Contact::get()->pluck('first_name', 'id')->prepend(trans('global.app_please_select'), '');
        $created_bies = \App\User::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');$zipcodes = \App\Zipcode::where('location_id', $id)->get();$tracking_numbers = \App\TrackingNumber::where('location_id', $id)->get();
